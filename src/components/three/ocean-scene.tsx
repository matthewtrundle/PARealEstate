...
function SunGlow() {
  return (
    <mesh position={[8, 4, -10]}>
      ...
      <meshBasicMaterial color="#ffeedd" transparent opacity={0.6} />
    </mesh>
  )
}
...
export function OceanScene({ className = "" }: OceanSceneProps) {
  return (
    <div className={`w-full h-full ${className}`}>
      ...
      >
        ...
        <directionalLight position={[8, 10, 5]} intensity={1} color="#ffeedd" />
        <directionalLight position={[-5, 5, -5]} intensity={0.3} color="#88bbff" />
        ...
        {/* Fog */}
        <fog attach="fog" args={["#0f1e32", 5, 25]} />
      </Canvas>
    </div>
  )
}
